...
#endif
{
        ...
        if(r < PROB_MEMALIGN) {
                ...
                if(m->size == 0) {
                        ...
                        m->ptr = (unsigned char *)my_malloc(1);
#endif
                }
#if TEST > 2
                key = RANDOM(256);
...
#endif
        } else { /* normal my_malloc call */
                if(m->size > 0) free(m->ptr);
                m->size = random_size(size);
                m->ptr = (unsigned char *)my_malloc(m->size);
        }
        if(!m->ptr) {
 ...
 * Local variables:
 * tab-width:4
 * compile-command: "gcc -Wall my_malloc-test.c -o my_malloc-test"
 * End:
 */
